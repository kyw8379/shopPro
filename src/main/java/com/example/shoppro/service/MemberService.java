...
public class MemberService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        ...
        return User.builder()
                .username(member.getEmail())      // session에 저장됨
                .password(member.getPassword())   // 로그인 시 입력한 비밀번호 와 비교할 값
                .roles(member.getRole().name())
                .build();
    }

    // 회원가입
    public Member saveMember(MemberDTO memberDTO) throws IllegalAccessException {
        // 회원가입 여부 확인
    ...
    // 메소드를 자체 제작
     private void validateDuplicateMember(String email) throws IllegalAccessException {
        ...
        memberRepository.findByEmail(email);
         ...
         // 이 내용은 try()catch(IllegalAccessException e ) {
